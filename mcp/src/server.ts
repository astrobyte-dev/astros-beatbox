...
import { DASHBOARD_PORT, METER_UDP_PORT, DASHBOARD_HTML, SETS_DIR, RECORDINGS_DIR, AUDIO_DEVICE_FILE, SCOPE_ENABLED } from "./config.js";

const engine = new Engine();
const meter = new Meter();
const app = new Application(engine, { sets: SETS_DIR, recordings: RECORDINGS_DIR, device: AUDIO_DEVICE_FILE });
const rig = app.rig;
...
server.tool("status", "Report engine health, last fault, session/generation and the existing tracked slot state.", async () => text(JSON.stringify({
  ...
  tracking: "Legacy slot/code projection; arbitrary interpreter code may not be representable.",
